...
OA        put = request.get_json()
OA        if put is None or type(put) != dict:
            return jsonify({'error': 'Not a JSON'}), 400
OA        for key, value in put.items():
OA            if key not in ['id', 'created_at', 'updated_at',
                           'place_id', 'user_id']:
                setattr(review, key, value)
OA                storage.save()
OA        return jsonify(review.to_dict()), 200
